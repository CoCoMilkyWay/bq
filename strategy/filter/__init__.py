...
def apply_filter_intervals(df: pd.DataFrame, intervals_by_inst: dict) -> pd.DataFrame:
    """向量化应用过滤区间 (merge-based)"""
    df = df.reset_index(drop=True).copy()
    df["date_int"] = df["date"].dt.strftime("%Y%m%d").astype(int)

    rows = [(inst, s, e) for inst, ivs in intervals_by_inst.items() for (s, e) in ivs]
    assert rows, "intervals_by_inst is empty"
    iv = pd.DataFrame(rows, columns=["instrument", "s", "e"])

    df_key = df[["instrument", "date_int"]].reset_index(names="_row")
    merged = df_key.merge(iv, on="instrument", how="inner", copy=False)
    hit = merged.loc[
        (merged["date_int"] >= merged["s"]) & (merged["date_int"] <= merged["e"]),
        "_row",
    ].unique()

    keep_mask = ~df.index.isin(hit)
    result = df.loc[keep_mask].drop(columns=["date_int"]).reset_index(drop=True)
    return result
