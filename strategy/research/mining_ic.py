...
[阶段 1] 全量搜索: 对 lattice 上每个权重算 rank IC 均值 + ICIR (年化)
    触发: 1 次 batch 调用, 一次喂 P = C(n+M-1, n-1) 个权重
    kernel: evaluate_ic_csr  (parallel=True, prange over pop)
    每权重开销: 每日 (F 点积 + argsort(cnt) 得到 score 秩
                     + 与预计算的 ret 秩做 Pearson on ranks)
    ret 秩在 load 时对每日 argsort 一次, 全体 pop 共享 (不进 prange 热路径)
    ≈ 全部运行时间的 99%

[阶段 2] Top-N 后评估 (N=TOP_N, 合计 <1% 耗时, 基本可忽略):
    ...
    (2b) 年度分层单调度 Y     kernel: evaluate_monotonicity_csr
         触发: 1 次 batch 调用, pop = N (top N 权重)
         目的: 展示 "每年 Q1..QG 是否单调" 的分层能力 (与 IC 互补)
         单点成本与 IC kernel 同量级, 只 N 次

    (2c) 参数平原敏感度       函数: neighbor_indices (纯 Python, 非 kernel)
         触发: top N 逐个 BFS 找 [1, NEIGHBOR_DISTANCE_MAX] 跳邻居
               一跳 = 某 k_i -1 & 另一 k_j +1 (L1=2), 单步邻居上限 n*(n-1)
         目的: 邻居 IC 均值 vs 中心 IC 的衰减, 判断是否过拟合山尖
         不重跑评估, 只从阶段 1 的 ic 数组里查表, 开销忽略

    (2d) 最优权重年度档位表   kernel: evaluate_year_group_matrix_csr
         触发: 1 次调用, 仅对 top1 权重 (按 IC 排序)
         目的: 打印每年每档 (Q1..QG) 的累计收益 + 该年单调度, 便于直观检查

============================================================
Fitness 定义 (阶段 1): 日频 rank IC 均值, ICIR 年化
============================================================
    每日: scores = ranks @ w, 截面对 scores 与 fwd_ret 各自取 ordinal 秩
          (fwd_ret 的秩在 load 时预计算一次, 全体 pop 共享)
          rho_d = Pearson(score_rank, ret_rank)
                = Σ (Rs - μ)(Rr - μ) / (n(n²-1)/12)   (μ = (n-1)/2)
          无并列时等价于 Spearman; 有并列 (ret 可能 = 0) 时是近似.
    IC    = mean_d(rho_d)              (跨活跃日算术平均)
    ICIR  = mean_d(rho_d) / std_d(rho_d) * sqrt(252)  (年化)
    排序: 主排序指标为 IC (均值).
...
def load_data_from_file(input_path: Path = DATA_FILE):
    ...
    # 预计算 fwd_ret 的每日 ordinal 秩 (只依赖 rets, 不随 w 变, 所有 pop 共享)
    ret_ranks = np.empty(rets.shape[0], dtype=np.int32)
    D_total = len(off) - 1
    arange_buf = np.arange(int(max(1, np.diff(off).max())), dtype=np.int32)
    for _d in range(D_total):
        _lo = int(off[_d]); _hi = int(off[_d + 1])
        if _hi > _lo:
            _order = np.argsort(rets[_lo:_hi])
            ret_ranks[_lo + _order] = arange_buf[:_hi - _lo]
...
def evaluate_ic_csr(
    pop,           # (n_pop, F) float32
    ranks,         # (N, F) float32 C-order
    ret_ranks,     # (N,) int32, 预计算的每日 fwd_ret ordinal 秩
    off,           # (D+1,) int32
    active_day,    # (D,) uint8
    max_cnt,       # int
    ann_factor,    # float: ICIR 年化因子 sqrt(trading_days_per_year)
):
    """
    日频 rank IC. 每日:
        scores = ranks @ w, 对 scores argsort 得到 score 秩 (0..cnt-1)
        与预计算的 ret_ranks 做 Pearson on ordinal ranks (两边 σ 相同, 分母公式化)
        rho_d = cov / (cnt*(cnt²-1)/12)
    累积 sum_ic, sum_ic_sq, n_days, 输出:
        [:,0] = mean_d(rho_d)                            (IC)
        [:,1] = mean_d / std_d * ann_factor              (ICIR 年化)
        std_d 用总体 std (分母 n_days, 非 n_days-1)
    cnt < 2 或 active_day==0 的日子跳过.
    """
    n_pop = pop.shape[0]
    F = pop.shape[1]
    D = off.shape[0] - 1
    out = np.empty((n_pop, 2), dtype=np.float64)

    for p in numba.prange(n_pop):
        w = pop[p]
        scores = np.empty(max_cnt, dtype=np.float32)
        score_rank = np.empty(max_cnt, dtype=np.int32)
        sum_ic = 0.0
        sum_ic_sq = 0.0
        n_days = 0
        for d in range(D):
            ...
            for i in range(cnt):
                score_rank[order[i]] = i
            mean_r = (cnt - 1) * 0.5
            cov = 0.0
            for i in range(cnt):
                cov += (score_rank[i] - mean_r) * (ret_ranks[lo + i] - mean_r)
            denom = cnt * (cnt * cnt - 1) / 12.0
            ic = cov / denom
            sum_ic += ic
            sum_ic_sq += ic * ic
            n_days += 1
        ...
        if n_days == 0:
            ...
            mean_ic = sum_ic / n_days
            var_ic = sum_ic_sq / n_days - mean_ic * mean_ic
            if var_ic <= 0.0:
                out[p, 0] = mean_ic
                out[p, 1] = 0.0
            else:
                out[p, 0] = mean_ic
                out[p, 1] = mean_ic / (var_ic ** 0.5) * ann_factor

    return out


_ANN_FACTOR_IC = float(np.sqrt(252.0))


def evaluate_ic(pop: np.ndarray, data: dict) -> np.ndarray:
    """日频 rank IC + 年化 ICIR. 返回 (n_pop, 2): [:,0]=IC, [:,1]=ICIR(年化)."""
    pop = np.ascontiguousarray(pop, dtype=np.float32)
    return evaluate_ic_csr(
        pop,
        data["ranks"], data["ret_ranks"], data["off"], data["active_day"],
        data["max_cnt"], _ANN_FACTOR_IC,
    )
...
def run_grid_search(data: dict, top_n: int | None = None) -> tuple[np.ndarray, float, list[str]]:
    """
    两阶段评估:
      1) 全量 lattice 扫 rank IC + 年化 ICIR (主排序指标 = IC)
      2) Top-K 权重额外算:
         - 年度分层单调度 Y (分层能力, 与 IC 互补)
         - 粘性+扣费口径的全期多头 NAV 和多空 NAV
         - 邻居 IC 均值 (L1=2, 参数平原 / 过拟合敏感度)
    返回: (最优权重(全因子维度, 已归一化), 最优 IC, 搜索因子名列表)
    """
    ...
    ic_arr = np.empty(len(w_grid), dtype=np.float64)
    icir_arr = np.empty(len(w_grid), dtype=np.float64)
    pbar = tqdm(total=len(w_grid), desc="IC 搜索", unit="point")
    for st, ed in iter_eval_slices(len(w_grid)):
        out = evaluate_ic(w_grid[st:ed], sub_data)
        ic_arr[st:ed] = out[:, 0]
        icir_arr[st:ed] = out[:, 1]
        pbar.update(ed - st)
    pbar.close()
    ...
    for i, gi in enumerate(top_idx):
        ...
        nbr_means[i] = float(np.mean(ic_arr[nbrs])) if nbrs else float("nan")
        decays[i] = float(ic_arr[gi]) - nbr_means[i]
    ...
    print("列含义:")
    print("  #        : 在 lattice 内按 IC 降序的名次")
    print("  IC       : 日频 rank IC 均值 (Spearman 近似; 活跃日算术平均)")
    print("  ICIR     : IC 均值 / IC 标准差 * sqrt(252)  (年化 IR)")
    print(f"  NbrIC    : [1,{NEIGHBOR_DISTANCE_MAX}] 跳内全部邻居权重的 IC 均值 (扰动稳定性)")
    print("  衰减     : IC - NbrIC, 越接近 0 越抗过拟合 (山尖 vs 平原)")
    ...
    print("  Y        : 年均分层单调度 ∈ [0,1] (1.0 = 每年 Q1..QG 都完美单调)")
    ...
    header = (
        f"{'#':<3} {'IC':>7} {'ICIR':>6} {'NbrIC':>7} {'衰减':>7} {'星':<{star_w}} "
        f"{'N':>{nbr_col_w}} {'Y':>6} {'多头累计':>8} {'多头年均':>8} {'多空累计':>8} {'多空年均':>8}  权重"
    )
    print(header)
    print("-" * len(header))
    for rank, gi in enumerate(top_idx, 1):
        ...
        print(
            f"{rank:<3} {ic_arr[gi]:+7.4f} {icir_arr[gi]:+6.2f} {nbr_means[i]:+7.4f} {decays[i]:+7.4f} "
            f"{stars:<{star_w}} {int(nbr_counts[i]):{nbr_col_w}d} {float(top_y[i]):6.4f} "
    ...
    best_fitness = float(ic_arr[best_idx])
...
def main():
    ...
    print("主指标: 日频 rank IC 均值 (Spearman 近似), 年化 ICIR = IC/std * sqrt(252)")
    print(f"Top{TOP_N} 复评: 分层单调度 Y, 粘性+扣费 多头/多空 NAV (cost_rt={COST_ROUND_TRIP}), 邻居 IC 敏感度")
    print(f"SKIP_MONTHS={skip_disp} (命中日 IC/Y/NAV 均跳过, 持仓冻结)")
    print("=" * 60)
    ...
    _dummy[0, 0] = 1.0
    evaluate_ic(_dummy, data)
    evaluate_batch(_dummy, data)
    evaluate_monotonicity(_dummy, data)

    best_weights, best_ic, selected_names = run_grid_search(data)

    print("\n" + "=" * 60)
    print("最终结果")
    print("=" * 60)
    print(f"最优 IC (主排序指标): {best_ic:+.4f}")
    print(f"\n最优权重 (sum=1):")
