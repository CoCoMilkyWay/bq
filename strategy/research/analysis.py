...
def analyze_factor(df: pd.DataFrame, factor_col: str, group_num: int = GROUP_NUM) -> dict:
    ...
    rolling_ic["rolling_250"] = ic_series.rolling(250, min_periods=1).mean()
    rolling_ic_std_250 = ic_series.rolling(250, min_periods=1).std()
    rolling_ic["rolling_icir_250"] = rolling_ic["rolling_250"] / rolling_ic_std_250.replace(0, np.nan)
...
def plot_ic_rolling(results: dict):
    ...
    n = len(factor_names)
    fig = make_subplots(rows=n, cols=1, subplot_titles=factor_names, vertical_spacing=0.05, specs=[[{"secondary_y": True}]] * n)

    for i, factor_name in enumerate(factor_names, 1):
        rolling_ic = results[factor_name]["rolling_ic"]
        fig.add_trace(go.Scatter(x=rolling_ic.index, y=rolling_ic["ic"], mode="lines", name="ic", opacity=0.2, showlegend=(i == 1)), row=i, col=1, secondary_y=False)
        fig.add_trace(go.Scatter(x=rolling_ic.index, y=rolling_ic["rolling_250"], mode="lines", name="rolling_250", showlegend=(i == 1)), row=i, col=1, secondary_y=True)
        fig.add_hline(y=0, line_dash="dash", line_color="red", line_width=1, row=i, col=1, secondary_y=True)
        ic_abs_max = rolling_ic["ic"].abs().max()
        fig.update_yaxes(range=[-ic_abs_max, ic_abs_max], row=i, col=1, secondary_y=False)
        fig.update_yaxes(range=[-0.05, 0.05], row=i, col=1, secondary_y=True)

    fig.update_yaxes(title_text="IC", secondary_y=False)
    fig.update_yaxes(title_text="MA250", secondary_y=True, zeroline=True, zerolinecolor="red", zerolinewidth=2)
